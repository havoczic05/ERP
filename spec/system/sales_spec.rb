...
RSpec.describe 'Sales', type: :system do
  ...
  describe 'index page' do
    ...
    it 'renders the FAB (pill-shaped floating button) for create action' do
      ...
      expect(page).to have_css('.fab span', text: 'Nuevo documento')
    ...
    it 'positions the FAB on mobile/tablet and shows inline button on desktop', :js do
      ...
      begin
        ...
        expect(fab).to have_text('Nuevo documento')
        expect(page).to have_no_css('.fab[aria-label]')
    ...
    context 'pagination' do
      it 'renders pagination nav when more than one page exists' do
        # Sales paginates at 10/page; create 16 sales to trigger a second page.
        16.times do |i|
          create(:sale, client: client, warehouse: warehouse,
        ...
        expect(page).to have_content('Mostrando 1–10 de 16')
      end
    end
  end
